...
def img_cb(msg):
    try:
        ...
        eye_cascade_path = "/home/rg26/opencv/data/haarcascades/haarcascade_eye.xml"

        #分類器の作成
        face_classify = cv2.CascadeClassifier(face_cascade_path)
        eye_classify = cv2.CascadeClassifier(eye_cascade_path)
        ...
        if len(faces) == 0:
            ...
            for (x, y, w, h) in faces:
                ...
                eyes = eye_classify.detectMultiScale(gray_face)

                if len(eyes) == 0:
                    rospy.logwarn("no eyes")
